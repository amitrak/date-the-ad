...
for (const ad of sorted) {
  ...
  if (ad.yearRange) tags.push("Campaign span"); else if (isVintage(ad.year)) tags.push("Vintage scoring");
  ...
  p(`### ${ad.yearRange ? ad.yearRange[0] + "–" + ad.yearRange[1] : ad.year} — ${ad.title}`);
  p("");
  p(`*${tags.join(" · ")}*`);
  p("");
...
p("## Scoring");
p("");
p("| Points | Modern (" + VINTAGE_CUTOFF + "–present) | Vintage (pre-" + VINTAGE_CUTOFF + ") |");
p("| --- | --- | --- |");
p("| 10 | exact | exact |");
p("| 9 | 1 year off | 1–2 years off |");
p("| 8 | 2 years off | 3–4 years off |");
p("| 7 | 3 years off | 5–6 years off |");
p("| 6 | 4 years off | 7–8 years off |");
p("| 5 | 5 years off | 9–10 years off |");
p("| 3 | 6–9 years off | 11–13 years off |");
p("| 2 | 10–12 years off | 14–16 years off |");
p("| 1 | 13–15 years off | 17–20 years off |");
p("| 0 | more than 15 | more than 20 |");
p("");
p("The vintage cutoff is computed as `currentYear - 100`, so it moves with time.");
p("");
const ranged = ADS.filter(a => a.yearRange);
if (ranged.length) {
  p("### Campaign ads");
  p("");
  p("A few ads ran as campaigns rather than debuting once, so any year inside the span scores");
  p("the full 10, with a steeper drop-off outside it: 1 year out = 8, 2–3 = 6, 4–5 = 2, beyond = 0.");
  p("");
  for (const a of ranged) p(`- **${a.title}** — ${a.yearRange[0]}–${a.yearRange[1]}`);
  p("");
}
...
p("| Vintage popover | This ad is over a century old, so it scores on a gentler curve: exact = 10, within 2 years = 9, 4 = 8, 6 = 7, 8 = 6, 10 = 5, 13 = 3, 16 = 2, 20 = 1. Newer ads use tighter margins. |");
p("| Campaign popover | This campaign ran for years rather than debuting once, so any year in the span scores the full 10. From there: 1 year out = 8, 2–3 = 6, 4–5 = 2. |");
